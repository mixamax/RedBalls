    ...
    function updateDots() {
        for (let i = 1; i < dots.length; i++) {
            ...
            for (let j = 0; j < dots.length; j++) {
                ...
                let dist =
                    Math.sqrt(delta.x * delta.x + delta.y * delta.y) || 1;
                let force = ((dist - config.sphereRad) / dist) * b.mass;

                if (j === 0) {
                    let alpha = config.mouseSize / (dist / 2);
        ...
        dots.map((dot) =>
            dot === dots[0] ? dot.draw(mouse.x, mouse.y) : dot.draw()
        );
    }
...
// <h1 align="center">This code is taken to study canvas from a wonderful channel  <a href="https://youtu.be/KEQsm2yL6Lg" target="_blank">ThreePixDroid</a>
